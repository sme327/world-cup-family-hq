...
def _consensus_html(m, match_picks: pd.DataFrame) -> str:
    ...
    if h_cnt == a_cnt:
        ...
        ht = m['home_team']
        verdict = f"👨‍👩‍👧‍👦 <b>{ht}</b> ({h_cnt}/{total})"
        vc = "#93C5FD"
    else:
        at = m['away_team']
        verdict = f"👨‍👩‍👧‍👦 <b>{at}</b> ({a_cnt}/{total})"
        vc = "#93C5FD"
    ...
    if total > 1:
        if h_cnt == 1:
            ...
            ht = m['home_team']
            upset = f"<div style='font-size:.72rem;color:#FB923C;margin-top:.1rem'>⚠️ Only {solo} picked {ht}</div>"
        elif a_cnt == 1:
            solo = match_picks[match_picks['picked_team'] == m['away_team']]['user_name'].iloc[0]
            at = m['away_team']
            upset = f"<div style='font-size:.72rem;color:#FB923C;margin-top:.1rem'>⚠️ Only {solo} picked {at}</div>"
...
def _compact_picks_row(m, match_picks: pd.DataFrame) -> str:
    """Inline avatar row for compact completed cards: 🐘🌻 Mexico · 🐰 S.Africa"""
    if match_picks.empty:
        return ""
...
if not _sched.empty:
    ...
    ht_short = nm['home_team']
    at_short = nm['away_team']
    grp = nm['group_letter']
    ...
    st.markdown(
        ...
        f"{hf} {ht_short} &nbsp;vs&nbsp; {af} {at_short}</div>"
...
if not completed_df.empty:
    ...
    for pt_date_val, day_grp in completed_df.sort_values(
        ...
        for _, m in day_grp.iterrows():
            ...
            # Compact card: HTML block + 1 button column
